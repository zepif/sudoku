#include "pepepoop.h" //функции генерации поля
...
void draw_field(int chosen) {
    ...
    draw_line(points[chosen].x - step, points[chosen].y - step, points[chosen].x + step, points[chosen].y - step,
              width * 15 / 10, 0, 128, 0);
    draw_line(points[chosen].x - step, points[chosen].y - step, points[chosen].x - step, points[chosen].y + step,
              width * 15 / 10, 0, 128, 0);
    draw_line(points[chosen].x - step, points[chosen].y + step, points[chosen].x + step, points[chosen].y + step,
    ...
    for (int i = 0; i < 3; i++) {
        for (int j = 3 * 9 * i; j < 3 * 9 * i + 9; j++) {
            draw_line(points[j].x - step, points[j].y - step, points[j].x - step, points[j].y + step, width * 20 / 10,
                      0, 0, 0);
        }
    }
    for (int i = 72; i < 81; i++)
        draw_line(points[i].x + step, points[i].y - step, points[i].x + step, points[i].y + step, width * 20 / 10, 0, 0,
                  0);
...
void translate() {
    for (int i = 0; i < 9; i++) {
        for (int j = 0; j < 9; j++) {
            if (gamov_field[i][j]) field[j * 9 + i] = gamov_field[i][j];
        }
    }
}
    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
            ...
            if (event.type == Event::MouseButtonPressed) {
                ...
                if (x >= points[0].x - step && y >= points[0].y - step && x <= points[80].x + step &&
                    y <= points[80].y + step) {
                    double min_dist = 1e9;
                    int new_square;
                    for (int i = 0; i < 81; i++) {
                    ...
                    if (square != -1) {
                        ...
                        int r = count_radius();
                        for (int i = 0; i < 9; i++) {
                            if ((x - dig_points[i].x - r) * (x - dig_points[i].x - r) +
                                    (y - dig_points[i].y - r) * (y - dig_points[i].y - r) <=
                                r * r) {
                                digit = i + 1;
                                // break;
                            }
                        ...
                        if (digit != -1 && !start[square]) {
                            field[square] = digit;
                            if (correct_field[square] != digit) wrong_fields[square] = 1, errors++;
                        }
                        // square = -1;
