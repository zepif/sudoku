...
{
  int Summ;
  for (ll i=0; i<9; i++) { Summ += TestField[n][i]; }
  return Summ;
}
...
{
  int Summ;
  for (ll i=0; i<9; i++) { Summ += TestField[i][n]; }
  return Summ;
}
...
{
  int Summ;
  for (ll i=sq1*3; i<(sq1+1)*3; i++) for (ll j=sq2*3; j<(sq2+1)*3; j++)  Summ += TestField[i][j];
  return Summ;
}
...
bool FieldRevision(vector<vector<int>> TestField){
  bool res=1;
  for (ll i=0; i<9; i++) {if (   RowSumm(TestField,i) != 45) {res = 0;}}
  for (ll i=0; i<9; i++) {if (ColumnSumm(TestField,i) != 45) {res = 0;}}
  for (ll i=0; i<3; i++) for (ll j=0; j<3; j++) {if (SquareSumm(TestField,i,j) != 45) {res = 0;}}
  return res;
}
